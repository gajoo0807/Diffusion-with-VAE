...
def sample(model, scheduler, train_config, diffusion_model_config,
           autoencoder_model_config, diffusion_config, dataset_config, vae, cond_input):
    r"""
    ...
    for i in tqdm(reversed(range(diffusion_config['num_timesteps']))):
        ...
        ims = (ims + 1) / 2
        grid = make_grid(ims, nrow=train_config['num_samples']) 
...
def infer(args, prob):
    ...
    with torch.no_grad():
        sample(model, scheduler, train_config, diffusion_model_config,
               autoencoder_model_config, diffusion_config, dataset_config, vae, prob)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', dest='config_path',
                        default='config/mnist_dist_cond.yaml', type=str)
    ...
    probabilities = load_distribution(args)
    infer(args, probabilities)
