...
class DownBlock(nn.Module):
    ...
    2. Attention block
    3. Downsample
    移除cross attention：text condition needed
    """
    ...
    def __init__(self, in_channels, out_channels, t_emb_dim,
        ...
        self.dist_emb_layers = nn.ModuleList([
                nn.Sequential(
                    nn.SiLU(),
                    nn.Linear(self.dist_emb_dim, out_channels)
                )
                for _ in range(num_layers)
        ])
...
class MidBlock(nn.Module):
    ...
    def forward(self, x, t_emb=None, dist=None):
        ...
        out = out + self.dist_emb_layers[0](dist)[:, :, None, None]
...
class UpBlockUnet(nn.Module):
    ...
    def forward(self, x, out_down=None, t_emb=None, dist=None):
        x = self.up_sample_conv(x)
        if out_down is not None:
            x = torch.cat([x, out_down], dim=1)
